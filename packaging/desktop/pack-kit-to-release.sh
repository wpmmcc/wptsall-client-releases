#!/usr/bin/env bash
# Public-side pack: consume a source-free desktop kit → user release assets.
# Does NOT cargo-build. Safe for free public runners.
#
# Expects KIT=.../kit-desktop-{platform}.tar.gz (see make-kit-desktop.sh).
set -euo pipefail
...
# Allow relative KIT when cwd is public-repo root or install-client
[ -f "$KIT" ] || KIT="$INSTALL_CLIENT/$KIT"
[ -f "$KIT" ] || { echo "ERROR: Kit not found: $KIT" >&2; exit 1; }

# Detect platform from kit filename (kit-desktop-linux-x86_64 → linux-x86_64)
PLAT="$(basename "$KIT" .tar.gz | sed 's/^kit-desktop-//;s/^kit-webui-//;s/^kit-//')"
echo "== pack-kit-to-release ver=$VER platform=$PLAT kinds=$PACK_KINDS =="

# Extract kit
STAGE="$INSTALL_CLIENT/dist/staging"
rm -rf "$STAGE"
mkdir -p "$STAGE"
tar -xzf "$KIT" -C "$STAGE"
TREE="$STAGE/wptsall-client"
[ -d "$TREE" ] || { echo "ERROR: kit root wptsall-client missing in $KIT" >&2; exit 1; }
